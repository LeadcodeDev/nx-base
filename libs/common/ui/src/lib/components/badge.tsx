...
import React, { ComponentPropsWithoutRef, ForwardedRef, forwardRef, PropsWithChildren } from 'react';
...
export function Badge(props: BadgeProps & ComponentPropsWithoutRef<'span'>) {
  const { color = 'zinc', className, ...params } = props
  return (
    <span
      {...params}
...
export const BadgeButton = forwardRef(function BadgeButton(props: PropsWithChildren<BadgeProps & (HeadlessButtonProps | ComponentPropsWithoutRef<typeof Link>)>,
  ...
  return 'href' in props ? (
    <Link {...props} className={classes} ref={ref as ForwardedRef<HTMLAnchorElement>}>
      <TouchTarget>
        <Badge color={props.color}>{props.children}</Badge>
      </TouchTarget>
    </Link>
  ) : (
