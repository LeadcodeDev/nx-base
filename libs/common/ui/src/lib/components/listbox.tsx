...
export function ListboxOption<T>({
  ...
  return (
    <HeadlessListboxOption as={Fragment} {...props}>
      {({ selectedOption }) => {
        if (selectedOption) {
          return <div className={classNames(className, sharedClasses)}>{children}</div>
        }
        ...
        return (
          ...
          >
            ...
            <span className={classNames(className, sharedClasses, 'col-start-2')}>{children}</span>
          </div>
        )
      }}
...
export function ListboxLabel({ className, ...props }: React.ComponentPropsWithoutRef<'span'>) {
  return <span className={classNames(className, 'ml-2.5 truncate first:ml-0 sm:ml-2 sm:first:ml-0')} {...props} />
}
