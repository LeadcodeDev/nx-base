  ...
  return (
    <HeadlessMenuItem
      ...
      className={classNames(
        ...
        // Base styles
        'group cursor-default rounded-lg px-3.5 py-2.5 focus:outline-none sm:px-3 sm:py-1.5 cursor-pointer',

        // Text styles
        'text-left text-base/6 text-zinc-950 sm:text-sm/6 dark:text-white forced-colors:text-[CanvasText]',

        // Focus
        'data-[focus]:bg-blue-400 data-[focus]:text-white',
...
export function DropdownHeader({ className, ...props }: ComponentPropsWithoutRef<'div'>) {
  return <div {...props} className={classNames(className, 'col-span-5 px-3.5 pb-1 pt-2.5 sm:px-3')} />
}
...
export function DropdownLabel({ className, ...props }: HeadlessLabelProps) {
  return (
    <HeadlessLabel {...props} data-slot="label" className={classNames(className, 'col-start-2 row-start-1')} {...props} />
  )
}
...
export function DropdownShortcut({
  ...
  return (
    <HeadlessDescription
      as="kbd"
      {...props}
      className={classNames(className, 'col-start-5 row-start-1 flex justify-self-end')}
    >
